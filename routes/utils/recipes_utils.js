...
async function getRecipeLikesCount(recipe_id) {
    try {
        ...
        const userLikes = parseInt(userLikesResult[0].userLikes) || 0;
        ...
        try {
            ...
            console.log(`Spoonacular likes for recipe ${recipe_id}:`, spoonacularLikes);
        } catch (apiError) {
            console.warn(`Failed to get Spoonacular likes for recipe ${recipe_id}:`, apiError.message);
            spoonacularLikes = 0;
        }
        
        const totalLikes = spoonacularLikes + userLikes;
        console.log(`Total likes for recipe ${recipe_id}:`, totalLikes);
        return totalLikes;
    } catch (error) {
        console.error("Error getting recipe likes count:", error);
        // Return 0 if there's an error to avoid breaking the app
        return 0;
    }
}
...
async function getRecipeDetailsWithLikes(recipe_id, user_id = null) {
    try {
        ...
        const userHasLiked = user_id ? await hasUserLikedRecipe(user_id, recipe_id) : false;
